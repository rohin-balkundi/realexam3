...
## 2 use tidy census to find gini
```{r}
...
```
## 5 show that inequality_panel and inequality_long have the same number of obs
```{r}
#5 show same number of obs
count(inequality_long) == count(inequality_panel)
```
## 6 collapse the inequality long df
```{r}
...
inequality_collapsed <- inequality_long %>%
  ...
  summarize(across(gini,mean))
```
## 7. produce a map of the US that colors in state polygons
```{r}
...
US_map = ggplot() +
  ...
  theme_void()
```
## 8 import the data and rename variable
```{r}
...
```
chose 2015 as a base year since it equated to 100

##10 3 parts to a shiny app
Inputs <- what the user puts in (can be buttons, checkboxes or blanks)
Outputs <- what the user is given (can be an image, a graph, or a table)

Server 
dictates how to output the information to the user
always store directions in an object
use  the render fuction to output the object
refer to the output in your input
...
```
### 13. tokenize and run stop words
```{r}
library(tidytext)
data(stop_words)
...
<https://github.com/rohinbal/realexam3/tree/master/EXAM2>